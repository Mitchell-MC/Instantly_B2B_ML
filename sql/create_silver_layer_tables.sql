-- Silver Layer Tables for ML Lead Scoring System
-- Production-ready schema for processed ML features

-- First, create the ml_lead_scoring schema if it doesn't exist
...
-- Create silver layer table for ML features
CREATE TABLE IF NOT EXISTS ml_lead_scoring.silver_ml_features (
    -- Primary identifiers
    id TEXT PRIMARY KEY,
...
CREATE INDEX IF NOT EXISTS idx_silver_ml_features_email ON ml_lead_scoring.silver_ml_features(email);
CREATE INDEX IF NOT EXISTS idx_silver_ml_features_campaign ON ml_lead_scoring.silver_ml_features(campaign);
CREATE INDEX IF NOT EXISTS idx_silver_ml_features_engagement_level ON ml_lead_scoring.silver_ml_features(engagement_level);
CREATE INDEX IF NOT EXISTS idx_silver_ml_features_processed_timestamp ON ml_lead_scoring.silver_ml_features(processed_timestamp);
CREATE INDEX IF NOT EXISTS idx_silver_ml_features_data_quality ON ml_lead_scoring.silver_ml_features(data_quality_score);
CREATE INDEX IF NOT EXISTS idx_silver_ml_features_composite ON ml_lead_scoring.silver_ml_features(email, campaign, processed_timestamp);

-- Apollo-specific indexes for B2B analysis
CREATE INDEX IF NOT EXISTS idx_silver_ml_features_apollo_enrichment ON ml_lead_scoring.silver_ml_features(has_apollo_enrichment);
CREATE INDEX IF NOT EXISTS idx_silver_ml_features_company_size ON ml_lead_scoring.silver_ml_features(company_size_category);
CREATE INDEX IF NOT EXISTS idx_silver_ml_features_company_age ON ml_lead_scoring.silver_ml_features(company_age_category);
CREATE INDEX IF NOT EXISTS idx_silver_ml_features_high_value ON ml_lead_scoring.silver_ml_features(is_high_value_title, is_high_value_seniority);
CREATE INDEX IF NOT EXISTS idx_silver_ml_features_apollo_quality ON ml_lead_scoring.silver_ml_features(apollo_data_completeness_pct);
CREATE INDEX IF NOT EXISTS idx_silver_ml_features_apollo_freshness ON ml_lead_scoring.silver_ml_features(apollo_data_freshness);

-- Create trigger for automatic timestamp updates
CREATE OR REPLACE FUNCTION update_silver_ml_features_timestamp()
RETURNS TRIGGER AS $$
BEGIN
...
CREATE TRIGGER update_silver_ml_features_timestamp
    BEFORE UPDATE ON ml_lead_scoring.silver_ml_features
    FOR EACH ROW EXECUTE FUNCTION update_silver_ml_features_timestamp();

-- Create view for ML training data (excludes identifiers)
CREATE OR REPLACE VIEW ml_lead_scoring.v_ml_training_features AS
SELECT 
    -- Target variable
    engagement_level,
...
CREATE MATERIALIZED VIEW IF NOT EXISTS ml_lead_scoring.mv_ml_training_summary AS
SELECT 
    engagement_level,
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_mv_ml_training_summary_engagement 
ON ml_lead_scoring.mv_ml_training_summary(engagement_level);

-- Grant permissions (adjust as needed for your environment)
GRANT SELECT ON ml_lead_scoring.silver_ml_features TO ml_read_role;
GRANT SELECT ON ml_lead_scoring.v_ml_training_features TO ml_read_role;
GRANT SELECT ON ml_lead_scoring.mv_ml_training_summary TO ml_read_role;

-- Comments for documentation
COMMENT ON TABLE ml_lead_scoring.silver_ml_features IS 'Silver layer ML-ready features derived from bronze layer data';
COMMENT ON COLUMN ml_lead_scoring.silver_ml_features.engagement_level IS 'Target variable: 0=no engagement, 1=opened, 2=clicked/replied';
COMMENT ON COLUMN ml_lead_scoring.silver_ml_features.data_quality_score IS 'Overall data quality score (0-1, higher is better)';
COMMENT ON VIEW ml_lead_scoring.v_ml_training_features IS 'Training-ready features excluding PII and identifiers';
