"""
Enhanced RandomForest Binary Classification with Advanced Feature Engineering
Implements senior data scientist recommendations for improved performance.
...
def create_ensemble_model(X_train, y_train):
    ...
    # Base models with optimal parameters
...
def main():
    ...
    print("=== ENHANCED RANDOMFOREST: EMAIL OPENING PREDICTION ===")
    ...
    # 4. Apply SMOTE for better class balance
    ...
    ensemble_model = create_ensemble_model(X_train_balanced, y_train_balanced)
    ...
    # 7. Feature importance (from RandomForest component)
    ...
    plt.title('Top 20 Feature Importances - Enhanced RandomForest Ensemble\n(Opening vs Not Opening Prediction)')
    plt.tight_layout()
    plt.savefig('enhanced_randomforest_feature_importance.png', dpi=300, bbox_inches='tight')
    ...
    plt.title('Enhanced Confusion Matrix - RandomForest Ensemble\nOpening Prediction')
    plt.ylabel('Actual')
    plt.xlabel('Predicted')
    plt.xticks([0, 1], ['Not Opened', 'Opened'])
    plt.yticks([0, 1], ['Not Opened', 'Opened'])
    plt.tight_layout()
    plt.savefig('enhanced_randomforest_confusion_matrix.png', dpi=300, bbox_inches='tight')
    print(f"💾 Enhanced confusion matrix saved")
    # plt.show()  # Commented out to avoid GUI issues
    
    # 10. Performance comparison and insights
    ...
    print(f"✅ Ensemble model (RandomForest + XGBoost + LogisticRegression)")
    print(f"✅ SMOTE class balancing applied")
    ...
    print(f"")
    print(f"🎉 TARGET ACHIEVED:" if improvement_auc > 0.02 else "📈 PROGRESS MADE:")
